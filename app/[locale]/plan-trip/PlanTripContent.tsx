...
export default function PlanTripContent() {
  ...
  const savedCategories = [
    ...new Set(savedDestinations.flatMap((d) => d.categories)),
  ];
    ...
    <div className="wrapper py-6 md:py-8">
      <div className="relative w-full h-64 sm:h-72 md:h-80 lg:h-96 overflow-hidden rounded-2xl">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 gap-4">
          ...
          <p className="text-white/80 text-base sm:text-lg max-w-lg md:max-w-xl px-3 sm:px-0">
            {t("description")}
          </p>
        ...
        <div className="flex flex-col gap-6 p-6 md:p-8 rounded-3xl shadow-sm bg-(--bg-primary) border border-gray-100">
            ...
            <div className="flex gap-2 flex-wrap">
              {[1, 2, 3, 4, 5, 6, 7].map((day) => (
                <button
                  key={day}
                  onClick={() =>
                    setLocalInputs((prev) => ({ ...prev, days: day }))
                  }
            ...
            <div className="grid grid-cols-3 gap-3">
              {(["low", "medium", "luxury"] as const).map((tier) => (
                <button
                  key={tier}
                  onClick={() =>
                    setLocalInputs((prev) => ({ ...prev, budget: tier }))
                  }
          ...
          <div className="flex flex-col gap-3">
            ...
            <Select
              ...
              onValueChange={(val) =>
                setLocalInputs((prev) => ({ ...prev, month: Number(val) }))
              }
            >
            ...
            <div className="grid grid-cols-3 gap-3">
              {(["relaxed", "balanced", "packed"] as const).map((intensity) => (
                <button
                  key={intensity}
                  onClick={() =>
                    setLocalInputs((prev) => ({ ...prev, intensity }))
                  }
                  className={`py-3 rounded-xl text-sm font-semibold transition-all duration-200 hover:opacity-90
                    ${
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              {["culture", "beach", "nature", "food", "mountain", "desert"].map(
                (cat) => (
                  <button
                    key={cat}
                    onClick={() => {
